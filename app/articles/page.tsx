...
const page: React.FC<SearchPageProps> = async({searchParams})=>{
    ...
    return(
        <div className="min-h-screen bg-background">
            <main className="container mx-autopx-4 py-12 sm:px-6 lg:px-8">
                {/* Page Header */}
                <div className="mb-12 space-y-6 text-center">
                    <h1 className="text-4xl font-bold text-purple-200 tracking-tight text-foreground sm:text-5xl">All Articles</h1>
                    {/* Search bar */}
                    <Suspense>
                        <ArticleSearchInput/>
                    </Suspense>
                ...
                <div className="mt-12 flex justify-center gap-2">
                    ...
                    {Array.from({length:totalPages}).map((_, index)=>(
                        ...
                        >
                        <Button
                        variant={`${currentPage === index+1? 'destructve':'ghost'}`}
                        size="sm"
